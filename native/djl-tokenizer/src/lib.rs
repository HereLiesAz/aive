...
// Vendored from deepjavalibrary/djl v0.38.0, extensions/tokenizers/rust/src/lib.rs.
// Aive changes: kept only the TokenizersLibrary JNI surface (dropped the Candle engine modules
// and RustLibrary) and made the build Android-only, so the library can be linked with 16 KB
// page alignment. The Java side (ai.djl.huggingface:tokenizers 0.38.0) loads it as
// "djl_tokenizer" on Android.
...
>(
    ...
    // Hub downloads are not supported on Android; load tokenizer.json via createTokenizerFromString.
