...
current thought
    ↓
related memory tags become available
    ↓
...
The memory system should **not** begin by injecting full remembered passages into an agent's context. In many cases, a few related tags are enough to make the active model remember what it was doing, what else it intended to do, or which resources it has already used.
...
It should surface the same way as other related memory: **tags first**.

For the Wasm example, the active agent may receive something as small as:

```text
Related memory tags: Web Wasm, test, router
```
...
Only when the tags are insufficient should the agent descend into more detailed memory representations.

---

## Tag-first retrieval hierarchy
...
Tags are therefore more than search indexes. They are **minimal associative memory cues**.
...
Tag-first surfacing applies equally to facts, work state, procedures, resources, plans, and deliberately banked notes.

An agent that has become narrowly focused may not need its entire previous plan injected. It may only need to see tags such as:
...
- similarity threshold required before tags surface
- frequency of associative tag injection
- number of tags injected at once
- activation decay
- repeat suppression / refractory period
- breadth of associative neighborhood considered

The default injected payload should remain small. Increasing the dial should normally make **related tags** appear more readily, not cause full memory passages to be dumped into context.
...
MEMORY GRAPH
    stores provenance and semantic associations
            ↓
ATTENTION LAYER
    decides how readily related tags enter awareness
            ↓
ACTIVE AGENT
    often recollects from tags alone
            ↓ only if needed
EXPLICIT MEMORY DRILL-DOWN
    phrase → summary → granular context → source
```

The durable rule is:

> **Bank experience. Organize it clerically. Surface associations as tags first. Let the active agent decide whether it needs to remember more.**
